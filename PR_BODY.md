...
> Note: `test_in_docker.sh` could not be executed locally (Docker not installed), but the Ubuntu CI job will run it if Docker is available on the runner.

## Checklist

- [x] Code compiles locally
- [x] Unit tests pass (`swift test`)
- [x] Stress tests pass locally
- [x] CHANGELOG updated
- [x] CI workflow added (macOS + Ubuntu)
- [ ] Optionally: add lint/format in CI (follow-up)

## Suggested reviewers & labels
- Reviewers: @skreutzberger, @lgaches (or project maintainers)
- Labels: `bug`, `tests`, `ci`

---

If you'd like, I can request reviews and add labels after the PR is opened.